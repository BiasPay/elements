...
type Segment =
    ...
    | { type: "optional"; children: Segment[] };

type VariantItem =
    | { type: "literal"; value: string }
    | { type: "slot"; key: string; matcher: RegExp };
...
const DEFAULT_DEFINITIONS: PatternMaskDefinitions = {
    0: /\d/,
    A: /[A-Za-z]/,
};
...
function parsePattern(pattern: string): Segment[] {
    ...
    const parseSegments = (terminator?: string): Segment[] => {
...
function compileVariants(pattern: string, definitions: PatternMaskDefinitions): PatternVariant[] {
    const segments = parsePattern(pattern);

    const expand = (nodes: Segment[]): VariantItem[][] => {
        ...
        for (const node of nodes) {
            ...
            if (node.type === "slot") {
                ...
                if (!matcher) {
                    throw new Error(
                        `Missing definition for slot "${node.key}" in pattern "${pattern}"`,
                    );
                }
                variants = variants.map((items) => [
                    ...items,
...
function normalizeText(text: string, prepare?: (value: string) => string): string {
    return prepare ? prepare(text) : text;
}

function packWithVariant(
    variant: PatternVariant,
    parts: PackPart[],
    prepare?: (value: string) => string,
): PackResult {
    ...
    for (const part of parts) {
        for (const char of normalizeText(part.text, prepare)) {
            while (
                slotIndex < variant.items.length &&
...
function formatWithVariant(variant: PatternVariant, raw: string, appending: boolean): FormatResult {
    ...
    for (const item of variant.items) {
        ...
        const shouldInclude =
            (rawIndex <= threshold && rawIndex < totalSlots) || rawIndex === totalSlots;
...
function rawIndexFromPosition(boundaries: number[], position: number): number {
...
export class PatternMask {
    ...
    constructor(input: HTMLInputElement, options: PatternMaskOptions) {
        ...
        input.addEventListener("beforeinput", this.#onBeforeInput as EventListener, opts);
        input.addEventListener("input", this.#onInput as EventListener, opts);
    ...
    #applyRaw(raw: string, rawCursor: number, appending: boolean): boolean {
        ...
        if (formatted.value === previousValue) {
            this.#input.setSelectionRange(
                formatted.boundaries[clampedCursor],
                formatted.boundaries[clampedCursor],
            );
            return false;
        }
    ...
    #dispatchSyntheticInput(inputType: string, data: string | null) {
    ...
    #deleteBackward(selectionStart: number, selectionEnd: number) {
        const packed = this.#packCurrentValue();
        if (!packed) {
            return false;
        }

        let before = rawIndexFromPosition(
            formatWithVariant(packed.variant, packed.raw, true).boundaries,
            selectionStart,
        );
        const after = rawIndexFromPosition(
            formatWithVariant(packed.variant, packed.raw, true).boundaries,
            selectionEnd,
        );
    ...
    #deleteForward(selectionStart: number, selectionEnd: number) {
        const packed = this.#packCurrentValue();
        if (!packed) {
            return false;
        }
    ...
    #replaceSelection(selectionStart: number, selectionEnd: number, insertedText: string) {
        const packed = this.#packCurrentValue();
        if (!packed) {
            return false;
        }
    ...
    #onInput = (event: InputEvent) => {
        ...
        const formatted = formatWithVariant(
            packed.variant,
            packed.raw,
        ...
        this.#input.setSelectionRange(
            formatted.boundaries[rawCursor],
            formatted.boundaries[rawCursor],
        );
        this.#emitAccept(this.#input.value);
    };
