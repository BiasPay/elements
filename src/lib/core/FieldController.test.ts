...
function setup(type: "cardNumber" | "cardExpiry" = "cardNumber") {
...
function authenticate(postMessage: ReturnType<typeof vi.fn>) {
...
describe("secure field channel", () => {
    ...
    it("queues outbound messages until the private port reports ready", () => {
        ...
        expect(
            channels[0]!.port1.postMessage.mock.calls.filter(
                ([message]) => message.type === "bias:authenticate-field",
            ),
        ).toHaveLength(1);
