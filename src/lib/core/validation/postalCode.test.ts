...
describe("validatePostalCode", () => {
    ...
    describe("US (vendored metadata)", () => {
        it("accepts 5-digit and ZIP+4 codes", () => {
            ...
            expect(validatePostalCode("22162-1010", "US", { showRequired: false }).valid).toBe(
                true,
            );
        });
    ...
    describe("CA", () => {
        it("accepts valid postal codes, with or without the space", () => {
            expect(
                validatePostalCode("M1A 1T1", "CA", { showRequired: false }, CA_METADATA).valid,
            ).toBe(true);
            expect(
                validatePostalCode("K1A0B1", "CA", { showRequired: false }, CA_METADATA).valid,
            ).toBe(true);
        });
        ...
        it("rejects invalid formats", () => {
            ...
            expect(
                validatePostalCode("D1A 1T1", "CA", { showRequired: false }, CA_METADATA).valid,
            ).toBe(false);
        });
    });
    ...
    describe("GB", () => {
        it("accepts valid postcodes", () => {
            for (const code of ["EC1A 1BB", "W1A 0AX", "M1 1AE"]) {
                expect(
                    validatePostalCode(code, "GB", { showRequired: false }, GB_METADATA).valid,
                ).toBe(true);
            }
        });
