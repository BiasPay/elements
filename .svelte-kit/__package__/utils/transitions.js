...
function scaleTranslate(value, factor) {
    return value
        .trim()
        .split(/\s+/)
        .map((part) => {
 ...
 */
export function collapse(node, params = {}) {
    const { delay = 0, duration = 300, easing = cubicOut, axis = "y", opacity = 0, scale = 1, translate, } = params;
    ...
    return {
        ...
        css: (t, u) => {
            ...
            if (scale !== 1)
                css += `scale: ${scale + (1 - scale) * t};`;
            if (translate)
                css += `translate: ${scaleTranslate(translate, u)};`;
            return css;
        },
    };
}
/** Fade + scale with no size collapse (e.g. the submit-button lock icon). */
export function fadeScale(node, params = {}) {
