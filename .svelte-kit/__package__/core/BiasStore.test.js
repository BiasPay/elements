...
import { createBiasStore, createInitialFieldState, initialFrameField, initialValueField, ADDRESS_VALUE_FIELDS, CARD_FRAME_FIELDS, CARD_VALUE_FIELDS, BANK_FRAME_FIELDS, BANK_VALUE_FIELDS, } from "./BiasStore";
...
vi.stubGlobal("fetch", vi.fn(async (url) => {
    if (String(url).includes("ssl-address"))
...
describe("createBiasStore", () => {
    ...
    describe("setAddressFields", () => {
        ...
        it("adds address fields to the required value fields without duplicates", () => {
            ...
            expect(new Set(required)).toEqual(new Set([...CARD_VALUE_FIELDS, ...ADDRESS_VALUE_FIELDS]));
    ...
    describe("shipping scope", () => {
        const SHIPPING_FIELDS = ADDRESS_VALUE_FIELDS.map((f) => f === "name"
            ? "shippingName"
            : f === "country"
                ? "shippingCountry"
                : f === "postalCode"
        ...
        it("registers shipping-scoped required fields", () => {
            ...
            expect(required.has("addressLine1")).toBe(false);
        });
        ...
        it("supports a billing and a shipping form at once", () => {
            ...
            expect(required.has("shippingAddressLine1")).toBe(true);
            store.setAddressFields("shipping", null);
            const afterUnmount = new Set(store.getRequiredValueFields());
            expect(afterUnmount.has("addressLine1")).toBe(true);
            expect(afterUnmount.has("shippingAddressLine1")).toBe(false);
        });
