...
import { getAddressMetadata, getAddressMetadataStatus, loadAddressMetadata, resolveAddressMetadata, } from "./address-metadata";
import { createBiasStore } from "./BiasStore";
import { elementsError, normalizeSessionError } from "./errors";
import { resolveElementsConfig } from "./endpoints";
import { scopedField, } from "./types";
function warnInactive(operation) {
    console.warn(`Bias Elements: ${operation}() was ignored because the controller is inactive.`);
}
function supportedMethods(session) {
    if (!session)
        return [];
    ...
    const values = session.payment_method_types ?? ["card"];
    if (!Array.isArray(values))
        return [];
    return values.filter((method) => method === "card" || method === "us_bank_account");
}
...
export class BiasController {
    ...
    generation = 0;
    request;
    initialSeedConsumed = false;
    completionGeneration = -1;
    collectors = new Map();
    frames = new Map();
    validatorTokens = new Map();
    paymentSurface;
    reconcilingSubmission = false;
    suppressSubmissionTransitions = false;
    terminalError;
    constructor(config) {
    ...
    activate() {
        ...
        if (seed && seed.client_secret !== this.config.clientSecret) {
            this.setTerminalError(elementsError("configuration_error", "initialCheckoutSession does not belong to clientSecret."));
            return;
        }
        this.applyRegistrations();
        void this.store.applyGeoDefaults();
        if (seed)
            this.applySession(seed);
    ...
    updateConfig(next) {
        ...
        const identityChanged = previous.clientSecret !== resolvedNext.clientSecret ||
            previous.apiBaseUrl !== resolvedNext.apiBaseUrl;
        const frameChanged = previous.frameUrl !== resolvedNext.frameUrl;
        if (resolvedNext.initialCheckoutSession !== previous.initialCheckoutSession &&
            !identityChanged) {
            console.warn("Bias Elements: a later initialCheckoutSession was ignored for the current checkout session.");
        }
        ...
        this.notify();
        if (this.active && identityChanged)
            void this.fetchSession(this.generation);
        if (this.active && ambiguousFrameChange)
            void this.fetchSession(this.generation);
    }
    refreshSession() {
        if (!this.active) {
    ...
    setPaymentMethod(method) {
        ...
        if (!supportedMethods(this.checkoutSession()).includes(method)) {
            console.warn(`Bias Elements: payment method "${method}" is not enabled for this session.`);
            return;
        }
    ...
    getField(name, scope = "billing") {
        const slot = this.resolveField(name, scope);
        const state = () => this.storeState.fieldState[slot];
        return {
            get state() {
                return {
                    value: state().value,
            ...
            setValue: (value) => {
                if (!this.active)
                    return warnInactive("setValue");
                if (this.submissionStatus === "submitting" || this.submissionStatus === "succeeded")
                    return;
                this.store.setFieldValue(slot, value);
            },
            setValidator: (validator) => this.registerValidator(slot, validator),
            validate: () => {
                if (this.active)
            ...
            onFocus: () => {
                if (this.active)
                    this.store.setValueFieldState(slot, { focused: true });
            },
            onBlur: () => {
                if (!this.active)
                    return;
    ...
    getFrame(field) {
        ...
        return {
            ...
            setState: (next) => {
                if (this.active)
                    this.store.setFrameFieldState(field, next);
            },
            setEncryptedData: (value) => {
                if (this.active)
                    this.store.onEncryptedData(field, value);
            },
        };
    }
    ...
    setBillingSameAsShipping(value) {
        if (!this.active)
            return warnInactive("setBillingSameAsShipping");
        if (this.submissionStatus === "submitting" || this.submissionStatus === "succeeded")
            return;
    ...
    registerCollector(collector) {
        ...
        if (this.active)
            this.applyRegistrations();
        return () => {
            if (!this.collectors.delete(token) || !this.active)
                return;
            this.applyRegistrations();
        };
    }
    registerFrame(binding) {
        const token = Symbol(binding.field);
        this.frames.set(token, binding);
        return () => {
            if (!this.frames.delete(token))
                return;
            binding.dispose?.();
        };
    }
    registerPaymentElement(method) {
        if (this.paymentSurface) {
            const error = elementsError("configuration_error", "Only one PaymentElement, CardElement, or USBankAccountElement may be mounted per provider.");
        ...
        if (method &&
            ...
            this.paymentSurface = undefined;
            const error = elementsError("configuration_error", `The mounted payment form uses "${method}", which is not enabled for this session.`);
            this.setTerminalError(error);
            throw error;
        }
        if (method && this.active)
            this.setPaymentMethod(method);
        return () => {
            if (this.paymentSurface?.token === token)
                this.paymentSurface = undefined;
        };
    }
    getAddressMetadata(scope) {
        const country = this.storeState.fieldState[scopedField("country", scope)].value;
        return this.getAddressMetadataForCountry(country);
    }
    getAddressMetadataForCountry(country) {
    ...
    async fetchSession(generation) {
        if (!this.active || generation !== this.generation || this.request)
            return;
        ...
        try {
            ...
            const result = await sdk.checkoutSessions.get("current", undefined, {
                signal: request.signal,
            });
            if (!this.owns(owner) || request.signal.aborted)
                return;
    ...
    applySession(session) {
        ...
        if (methods.length === 0) {
            this.setTerminalError(elementsError("configuration_error", "The checkout session has no supported payment methods."));
            return;
        }
        const fixedMethod = this.paymentSurface?.method;
        if (fixedMethod && !methods.includes(fixedMethod)) {
            this.setTerminalError(elementsError("configuration_error", `The mounted payment form uses "${fixedMethod}", which is not enabled for this session.`));
            return;
        }
        ...
        if (this.reconcilingSubmission) {
            ...
            this.submissionError = elementsError("payment_failed", "The payment result was reconciled and may now be retried.");
        }
        const nextMethod = fixedMethod ??
    ...
    relevantFieldChanged(previous, next) {
        for (const [name, state] of Object.entries(previous.fieldState)) {
            const nextState = next.fieldState[name];
        ...
        for (const field of Object.keys({
            ...
            if (previous.encryptedFields[field] !== next.encryptedFields[field])
                return true;
        }
        return false;
    }
    complete() {
        if (!this.active || this.submissionStatus !== "succeeded")
            return;
        if (this.completionGeneration === this.generation)
            return;
    ...
    resolveField(name, scope) {
        if (name.startsWith("shipping"))
            return name;
        return scopedField(name, scope);
    }
    registerValidator(slot, validator) {
        ...
        this.store.registerFieldValidator(slot, ((value) => {
            const result = validator(value);
            return { valid: result.isValid, error: result.error };
        }));
        return () => {
            if (this.validatorTokens.get(slot) !== token)
                return;
    ...
    applyRegistrations() {
        for (const scope of ["billing", "shipping"]) {
            const fields = [...this.collectors.values()].flatMap((collector) => collector.kind === "address" && collector.scope === scope ? collector.fields : []);
            this.store.setAddressFields(scope, fields.length ? [...new Set(fields)] : null);
        }
        this.store.setContactDetailsFields([...this.collectors.values()].some((collector) => collector.kind === "contact"));
    }
}
